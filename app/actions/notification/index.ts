...
export function modifyOrShowTransactionNotificationById({
  ...
  transaction,
  status,
}: { autodismiss: number; transaction: Record<string, unknown>; status: string }) {
  return {
...
export function replaceNotificationById(notification: Record<string, unknown> & { id: string | number }) {
  return {
...
export function showSimpleNotification({
  ...
  status,
  id,
}: { autodismiss?: number; title: string; description: string; status: string; id?: string | number }) {
  return {
    id,
