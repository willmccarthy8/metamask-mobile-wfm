...
// Setter Maps
const toBigNumber: Record<string, (n: unknown) => BigNumber> = {
  hex: (n) => new BigNumber(stripHexPrefix(n as string), 16),
  dec: (n) => new BigNumber(String(n), 10),
  BN: (n) => new BigNumber((n as { toString(base: number): string }).toString(16), 16),
};
...
const baseChange: Record<string, (n: BigNumber) => unknown> = {
  hex: (n) => n.toString(16),
...
const getBigNumber = (value: unknown, base: number): BigNumber => {
  if (!isValidBase(base)) {
...
const addCurrencies = (a: unknown, b: unknown, options: Record<string, unknown> = {}) => {
...
const subtractCurrencies = (a: unknown, b: unknown, options: Record<string, unknown> = {}) => {
  const { aBase, bBase, ...conversionOptions } = options;
...
const multiplyCurrencies = (a: unknown, b: unknown, options: Record<string, unknown> = {}) => {
...
const conversionGreaterThan = ({ ...firstProps }: ConversionOptions, { ...secondProps }: ConversionOptions) => {
...
const conversionLessThan = ({ ...firstProps }: ConversionOptions, { ...secondProps }: ConversionOptions) => {
  const firstValue = converter({ ...firstProps });
  const secondValue = converter({ ...secondProps });
...
const conversionMax = ({ ...firstProps }: ConversionOptions, { ...secondProps }: ConversionOptions) => {
...
const conversionGTE = ({ ...firstProps }: ConversionOptions, { ...secondProps }: ConversionOptions) => {
  const firstValue = converter({ ...firstProps });
  const secondValue = converter({ ...secondProps });
  return firstValue.greaterThanOrEqualTo(secondValue);
};

const conversionLTE = ({ ...firstProps }: ConversionOptions, { ...secondProps }: ConversionOptions) => {
  const firstValue = converter({ ...firstProps });
  const secondValue = converter({ ...secondProps });
  return firstValue.lessThanOrEqualTo(secondValue);
};

const toNegative = (n: unknown, options: Record<string, unknown> = {}) => multiplyCurrencies(n, -1, options);
